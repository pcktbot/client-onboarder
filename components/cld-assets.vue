      ...
      <b-col class="mb-2">
        <b-btn-group size="sm">
          ...
          <b-btn
            ...
            pill
            class="mr-2"
            @click="getImagesByFolder(`onboarding/${selectedId}`)"
          >
...
export default {
  mixins: [CloudinaryMixin, LocationsMixin],
  data () {
    return {
      selected: [],
      perPage: 12,
  ...
  computed: {
    ...
    selectedId () {
      return this.$store.state.locations.selected[0]
    }
  },
  watch: {
  ...
  created () {
    const path = `onboarding/${this.selectedId}`
    if (path) {
      this.getImagesByFolder(path)
    }
  },
